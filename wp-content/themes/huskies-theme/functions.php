...
function custom_reply_post_date( $args ) // $reply_id, $humanize, $gmt, $date, $time
{
    ...
    // August 4, 2012 at 2:37 pm
        ...
        $result = sprintf( _x( '%1$s at %2$s', 'date at time', 'huskies-theme' ), $date, $time );
    }
    return $result;
}
...
function custom_get_time_since( $args, $older_date, $newer_date ) {
		...
		// Setup the strings
		$unknown_text   = __( 'sometime',  'huskies-theme' );
		$right_now_text = __( 'right now', 'huskies-theme' );
		$ago_text       = __( '%s ago',    'huskies-theme' );
		...
		$chunks = array(
			array( 60 * 60 * 24 * 365 , __( 'year',   'huskies-theme' ), __( 'years',   'huskies-theme' ) ),
			array( 60 * 60 * 24 * 30 ,  __( 'month',  'huskies-theme' ), __( 'months',  'huskies-theme' ) ),
			array( 60 * 60 * 24 * 7,    __( 'week',   'huskies-theme' ), __( 'weeks',   'huskies-theme' ) ),
			array( 60 * 60 * 24 ,       __( 'day',    'huskies-theme' ), __( 'days',    'huskies-theme' ) ),
			array( 60 * 60 ,            __( 'hour',   'huskies-theme' ), __( 'hours',   'huskies-theme' ) ),
			array( 60 ,                 __( 'minute', 'huskies-theme' ), __( 'minutes', 'huskies-theme' ) ),
			array( 1,                   __( 'second', 'huskies-theme' ), __( 'seconds', 'huskies-theme' ) )
		);
		...
		// so there's only two bits of calculation below:
			...
			if ( !isset( $chunks[$i] ) ) {
				...
				if ( $i + 2 < $j ) {
					...
					// Add to output var
					if ( 0 != $count2 ) {
						$output .= ( 1 == $count2 ) ? _x( ',', 'Separator in time since', 'huskies-theme' ) . ' 1 '. $name2 : _x( ',', 'Separator in time since', 'huskies-theme' ) . ' ' . $count2 . ' ' . $chunks[$i + 1][2];
					}
				}
